...
const ColumnLeft = ({
  ...
  const paraIconStyle = useMemo(() => {
    return {
      height: propHeight,
    };
  }, [propHeight]);

  const textBlockIconStyle = useMemo(() => {
    return {
      height: propHeight1,
    };
  }, [propHeight1]);

  const reviewsFrameIconStyle = useMemo(() => {
    return {
      height: propHeight2,
    };
  }, [propHeight2]);
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start gap-[40px] min-w-[300px] max-w-full text-left text-base text-secondary-dark-50 font-paragraphs-para-18px-semibold mq750:gap-[20px]">
